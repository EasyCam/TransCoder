...
class TranslationService:
    ...
    def __init__(self, model: str = "qwen3:0.6b", ollama_host: str = "http://localhost:11434"):
        self.model = model
    ...
    def translate(
        ...
        return ToolResult(
            success=True,
            data={"translations": results, "source_lang": source_lang}
        )
...
请输出改进后的翻译。只输出改进后的译文，不要包含任何解释或其他内容。"""
        ...
        try:
            ...
            return ToolResult(
                success=True,
                data={"improved_translation": improved},
                metadata={"model": model}
            )
        except Exception as e:
            return ToolResult(success=False, error=str(e))
    ...
    def translate_iterative(
        ...
        return self.translate_with_reflection(
            source_text, source_lang, target_lang, model, iterations
        )
...
class VectorDBService:
    ...
    def add_translation_pair(
        ...
        try:
            ...
            return ToolResult(
                success=True,
                data={"total_pairs": len(self.metadata)}
            )
        except Exception as e:
            return ToolResult(success=False, error=str(e))
...
class TerminologyService:
    ...
    def add_term(
        ...
        try:
            ...
            return ToolResult(
                success=True,
                data={"total_terms": len(self.terminology)}
            )
        except Exception as e:
            return ToolResult(success=False, error=str(e))
    ...
    def evaluate(
        ...
        try:
            if "rouge" in metrics:
                ...
                results["rouge"] = {
                    k: {"fmeasure": v.fmeasure} for k, v in scores.items()
                }
        except ImportError:
